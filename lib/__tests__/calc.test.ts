...
describe("computeProperty — rollup precedence (spec §2, rule 2)", () => {
  ...
  it("falls back to the budget line's stored actual when no expense entry exists for that subcategory", () => {
    const budget = [budgetLine({ subcategory: "Insurance", category: "Holding Costs", estimated: D(4200), actual: D(2100) })];

    const result = computeProperty(budget, [], []);
    const row = result.byCat["Holding Costs"].rows.find((r) => r.subcategory === "Insurance")!;
    expect(row.actual.toNumber()).toBe(2100);
  });
});
...
describe("computeProperty — profit basis switch (spec §2)", () => {
  const costBudget: BudgetLineInput[] = [
    budgetLine({ subcategory: "Purchase Price", category: "Purchase Costs", estimated: D(500000), actual: D(500000) }),
    budgetLine({ subcategory: "Roofing", category: "Rehab Costs", estimated: D(20000), actual: D(18000) }),
  ];
  ...
  it("computes margin as profit / basis × 100", () => {
    const budget = [
      budgetLine({ subcategory: "Purchase Price", category: "Purchase Costs", estimated: D(100000), actual: D(100000) }),
