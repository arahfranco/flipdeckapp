...
export async function POST(req: Request) {
  ...
  const { address, mls, type, beds, baths, sqft, lotSize, stories, photoUrl, status, monthlyRent } = body;
  if (!address || !type || beds == null || baths == null || sqft == null || lotSize == null || stories == null) {
    return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
  }

  // Status is optional; default to Scouting. Validate against the enum so a bad
  // value can't be written.
  const chosenStatus = status && Object.values(Status).includes(status) ? (status as Status) : Status.SCOUTING;
  ...
  const property = await db.property.create({
    data: {
      ...
      monthlyRent: monthlyRent === "" || monthlyRent == null ? null : Number(monthlyRent),
